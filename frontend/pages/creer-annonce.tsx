...
const creerannonce = () => {
    ...
    return (
        <>
            <button onClick={() => setButton(buttonContent.btnObject)} className={button === buttonContent.btnObject ? createAddStyle.btniconeselected : createAddStyle.btnicone} data-tip data-for="object-tip" data-delay-show='500'>
                <FaHandHolding size={20}/>
            </button>
            <ReactTooltip id="object-tip" place="right" effect="float" arrowColor='black' delayShow={500}>Objet</ReactTooltip>
            <button onClick={() => setButton(buttonContent.btnService)} className={button === buttonContent.btnService ? createAddStyle.btniconeselected : createAddStyle.btnicone} data-tip data-for="service-tip" data-delay-show='500'>
            ...
            {subButtons &&
                subButtons.map((button, index) => {
                    return(
                        <button key={index} onClick={()=> setSubButton(button)} type="button" className={button === subButton ? createAddStyle.btnselected : createAddStyle.btn}>{button}</button> 
                    )
                })
            }
            <Form 
